...
import com.station.common.utils.JetagUtils;
...
//	RedisClientTemplate template;
	...
	public AjaxResponse<ShowPage<GprsConfigInfo>> getGprsConfigInfoListPage(
		...
		if(StringUtils.isNotEmpty(searchGprsConfigInfoPagingVo.getGprsIdOut())) {
			searchGprsConfigInfoPagingVo.setGprsIdOut("%"+searchGprsConfigInfoPagingVo.getGprsIdOut()+"%");
		}else {
			searchGprsConfigInfoPagingVo.setGprsIdOut(null);
		}
	...
	public AjaxResponse<ShowPage<GprsConfigInfoStation>> getGprsConfigInfoStationListPage(
		...
		if(StringUtils.isNotEmpty(searchGprsConfigInfoPagingVo.getGprsId())) {
			searchGprsConfigInfoPagingVo.setGprsId("%"+searchGprsConfigInfoPagingVo.getGprsId()+"%");
		}else {
			searchGprsConfigInfoPagingVo.setGprsId(null);
		}
	...
	/**
	 * 直接通过三级公司修改gprs_config_info 表中的参数
	 * @param gprsConfigInfo
	 * @return
	 */
	@RequestMapping(value = "/updateGprsConfigInfoByCompanyId", method = RequestMethod.POST)
	@ResponseBody
	@ApiOperation(value = "根据pk更新基站gprs参数表", notes = "根据pk更新基站gprs参数表，属性为null的不更新")
	public AjaxResponse<Object> updateGprsConfigInfoByCompanyId(@RequestBody GprsConfigInfo gprsConfigInfo) {
		...
		try {
			GprsConfigInfoMapper.updateByCompanyId(gprsConfigInfo);
		} catch (Exception e) {
			return ajaxResponse;
		}
		ajaxResponse.setCode(Constant.RS_CODE_SUCCESS);
		ajaxResponse.setMsg("修改gprs参数成功！");
		return ajaxResponse;
	}
	...
	public AjaxResponse<Object> updateByCompanyId(@RequestBody GprsConfigInfo gprsConfigInfo) {
		...
		queryStationInfo.setGprsIdNotNull(true);// 不查询为 -1 的
		...
		for (StationInfo stationInfo : stationList) {
			if (!String.valueOf(-1).equals(stationInfo.getGprsId())) {
				...
				//判断---排除检测设备和诊断设备进行均衡参数设置
				if (gprsInfo != null && gprsInfo.getDeviceType() != 3 && gprsInfo.getDeviceType() != 4 && gprsInfo.getDeviceType() != 2) {
					// ---add 11/1添加均衡信息
					// TODO ------------ 主机均衡策略参数配置 --------
					ModifyBalanceSend balanceSend = new ModifyBalanceSend();
					BeanUtils.copyProperties(gprsConfigInfo, balanceSend);
					balanceSend.setGprsId(gprsInfo.getGprsId());
					balanceSend.setSendDone(0);// 未发送
					balanceSend.setSendTime(null);
					balanceSend.setId(null);
					balanceSend.setGprsId(gprsInfo.getGprsId());
					boolean isBalanceSend = modifyBalanceInfoSer.insertParaNonNull(balanceSend);
					isParamSetSuccess = isParamSetSuccess || isBalanceSend;
				}
				//TODO --------start 运营参数设置（单体性能设置、电池组放电时长设置）会导致模型计算结果改变 ----------
...
//					GprsConfigInfo gprsInfo = CollectionUtils.isEmpty(configs) ? null : configs.get(0);
					...
					isParamSetSuccess = isParamSetSuccess || isCalc;
				}
				//TODO -----------end 运营参数设置 ----------------------------
			}
		}
		if (isParamSetSuccess) {
			ajaxResponse.setCode(Constant.RS_CODE_SUCCESS);
			ajaxResponse.setMsg("主机指令已发送！");
		}else {
			ajaxResponse.setCode(Constant.RS_CODE_ERROR);
			ajaxResponse.setMsg("主机指令发送失败！");
		}
		return ajaxResponse;
	}
	...
	/*
	 * 直接通过gprsid修改gprs表中的参数
	 */
	@RequestMapping(value = "/updateByGprsId", method = RequestMethod.POST)
	@ResponseBody
	@ApiOperation(value = "根据pk更新基站gprs参数", notes = "根据pk更新基站gprs参数，属性为null的不更新")
	public AjaxResponse<Object> updateByGprsId(@RequestBody GprsConfigInfo gprsConfigInfo) {
		if(gprsConfigInfo.getGprsId() == null) {
			return new AjaxResponse<Object>(Constant.RS_CODE_ERROR,"gprsId参数没有设置！");
		}
		AjaxResponse<Object> ajaxResponse = new AjaxResponse<Object>(Constant.RS_CODE_ERROR, "修改基站gprs参数配置出错！");
		try {
			gprsConfigInfoSer.updateByGprsSelective(gprsConfigInfo);
		} catch (Exception e) {
			return ajaxResponse;
		}
		ajaxResponse.setCode(Constant.RS_CODE_SUCCESS);
		ajaxResponse.setMsg("修改基站gprs参数成功！");
		return ajaxResponse;
	}
	...
	public AjaxResponse<Object> updateParam( @RequestBody GprsConfigInfo gprsConfigInfo) {
		AjaxResponse<Object> ajaxResponse = new AjaxResponse<>(Constant.RS_CODE_SUCCESS, "远程主机参数指令发送成功！");
		String str= "";
		//心跳间隔(秒)
	 ...
	 */
	@Override
	public boolean parseFile(File file, AjaxResponse ajaxResponse, Integer companyId,HttpServletRequest request)
	...
	public AjaxResponse<GprsConfigInfoDetail> getEntityDetail(@PathVariable Integer id) {
		...
		if (gprsConfigInfo != null) {
			...
			gprsConfigInfo.setVolLevel(device.get(0).getVolLevel());
			if(gprsConfigInfo.getSubDeviceList().get(0).getSubDeviceId() == null) {
				gprsConfigInfo.setSubDeviceList(null);
			}
	...
	public AjaxResponse<GprsConfigInfo> save(@RequestBody GprsConfigInfo gprsConfigInfo) {
		...
		if (matches) {
			...
			ajaxResponse.setMsg("设备编号不能有空格，字符长度不能超过9位并且满足第二位数必须是1的规则。例：T1B000111");
		}
	...
	public AjaxResponse<GprsConfigInfo> masterSave(@RequestBody GprsConfigInfo gprsConfigInfo) {
		...
		if(gprsConfigInfo.getSubDeviceCount() == null) {
			ajaxResponse.setMsg("该设备类型对应的从机个数没有设置！");
			return ajaxResponse;
		}
		// 主机正则表达式
		...
		if (matches) {
			...
			ajaxResponse.setMsg("设备编号不能有空格，字符长度不能超过9位并且满足第二数必须是0的规则，例：Y0A000001/T0B000002");
		}
	...
	public AjaxResponse getUnbindGprsList(@PathVariable String gprsId) {
		AjaxResponse ajaxResponse = new AjaxResponse(Constant.RS_CODE_SUCCESS, "查询成功");
		Pattern regex = Pattern.compile("^([a-z_A-Z_0-9]{0,9})$");
		boolean matcher = regex.matcher(gprsId).matches();
		if(!matcher) {
			ajaxResponse.setCode(Constant.RS_CODE_ERROR);
			ajaxResponse.setMsg("输入是搜索参数必须是数字或者字母，长度不超过9位的字符串！");
			return ajaxResponse;
		}
		gprsId = "%"+gprsId+"%";
		List unbindGprsList = gprsConfigInfoSer.selectUnbindGprsList(gprsId);
		ajaxResponse.setData(unbindGprsList);
		return ajaxResponse;
	}
	...
	public AjaxResponse<List<String>> getBindGprsList(@RequestBody GprsConfigInfo gprsConfigInfo) {
		...
		Pattern regex = Pattern.compile("^([a-z_A-Z_0-9]{0,9})$");
		boolean matcher = regex.matcher(gprsConfigInfo.getGprsId()).matches();
		if(!matcher) {
			ajaxResponse.setCode(Constant.RS_CODE_ERROR);
			ajaxResponse.setMsg("输入是搜索参数必须是数字或者字母，长度不超过9位的字符串！");
			return ajaxResponse;
		}
		gprsConfigInfo.setGprsId("%"+gprsConfigInfo.getGprsId()+"%");
		List<String> bindGprsList = gprsConfigInfoSer.selectBindGprsList(gprsConfigInfo);
		ajaxResponse.setData(bindGprsList);
		return ajaxResponse;
	}
