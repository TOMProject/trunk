...
public class CellVolLevelController extends BaseController{
	...
	private static final Logger logger = LoggerFactory.getLogger(CellVolLevelController.class);
	...
	public AjaxResponse<ShowPage<CellVolLevel>> getCellVolLevePaging(@RequestBody SearchCellVolLevelPagingVo searchCellVolLevelPagingVo){
		if(StringUtils.isNotEmpty(searchCellVolLevelPagingVo.getVolLevelName())) {
			searchCellVolLevelPagingVo.setVolLevelName("%"+searchCellVolLevelPagingVo.getVolLevelName()+"%");
		}else {
			searchCellVolLevelPagingVo.setVolLevelName(null);
		}
	...
	public AjaxResponse<Object> update(@RequestBody CellVolLevel cellVolLevel){
		...
		if(cellVolLevel.getVolLevelName() != null) {
			...
			logger.info(new Date()+"修改设备电压平台名称由"+volLevel.getVolLevelName()+"；修改为"+cellVolLevel.getVolLevelName());
		}
