...
public class SubDeviceController extends BaseController {
	...
	public AjaxResponse<ShowPage<SubDevice>> getSubDeviceListPage(@RequestBody SearchSubDevicePagingVo searchSubDevicePagingVo) {
		// add 查询出备用从的标志是备用的
		if(StringUtils.isNotEmpty(searchSubDevicePagingVo.getSubDeviceIdOut())) {
			searchSubDevicePagingVo.setSubDeviceIdOut("%"+searchSubDevicePagingVo.getSubDeviceIdOut()+"%");
		}else {
			searchSubDevicePagingVo.setSubDeviceIdOut(null);
		}
		searchSubDevicePagingVo.setSubFlag(1);//只是显示备用的
