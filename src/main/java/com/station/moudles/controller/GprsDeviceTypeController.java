...
public class GprsDeviceTypeController extends BaseController {
	...
	private static final Logger logger = LoggerFactory.getLogger(GprsDeviceTypeController.class);

	@RequestMapping(value = "/listPage", method = RequestMethod.POST)
	@ResponseBody
	@ApiOperation(value = "根据参数显示设备类型", notes = "返回设备类型列表")
	public AjaxResponse<ShowPage<GprsDeviceType>> getGprsDeviceTypePaging(
			@RequestBody SearchGprsDeviceTypePagingVo searchGprsDeviceTypePagingVo) {
		if(StringUtils.isNotEmpty(searchGprsDeviceTypePagingVo.getTypeName())) {
			searchGprsDeviceTypePagingVo.setTypeName("%"+searchGprsDeviceTypePagingVo.getTypeName()+"%");
		}else {
			searchGprsDeviceTypePagingVo.setTypeName(null);
		}
	...
	public AjaxResponse<List<GprsDeviceType>> getGprsDeviceTypeList(@RequestBody GprsDeviceType gprsDeviceType) {
		...
		AjaxResponse<List<GprsDeviceType>> ajaxResponse = new AjaxResponse<List<GprsDeviceType>>(
				Constant.RS_CODE_SUCCESS, "查询成功");
	...
	public AjaxResponse<Object> update(@RequestBody GprsDeviceType gprsDeviceType) {
		...
		if (gprsDeviceType.getTypeName() != null) {
			...
			if (!deviceType.getTypeName().equals(gprsDeviceType.getTypeName())) {
				GprsDeviceType typeName = gprsDeviceTypeMapper.selectDevieceType(gprsDeviceType.getTypeName());
				if (typeName != null) {
					ajaxResponse.setCode(Constant.RS_CODE_ERROR);
					ajaxResponse.setMsg("设备类型名称重复！");
					return ajaxResponse;
			...
			logger.info(new Date()+"修改设备类型的名称由："+deviceType.getTypeName()+"，修改为："+gprsDeviceType.getTypeName()+
					"从机电压由："+deviceType.getSubVol()+"，修改为："+gprsDeviceType.getSubVol()+"V，"
							+ "单体电压平台由："+deviceType.getVolLevel()+"V，修改为："+gprsDeviceType.getVolLevel()+"V");
		}
