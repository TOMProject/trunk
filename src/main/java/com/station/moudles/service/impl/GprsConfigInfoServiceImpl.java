...
public class GprsConfigInfoServiceImpl extends BaseServiceImpl<GprsConfigInfo, Integer>
	...
	public void updateByCompanyId(GprsConfigInfo gprsConfigInfo) {
		...
		if (!flag) {
			// 表示 GprsConfigSend 中有值, 肯定是 ‘远程主机参数配置’
			// TODO 2018.3.6 增加了逻辑更据电压平台设置
			if (gprsConfigInfo.getVolLevel() == null) {
				throw new RuntimeException("远程主机参数配置，未选择电压平台！");
			}
			GprsConfigInfo queryGprsConfigInfo = new GprsConfigInfo();
			queryGprsConfigInfo.setCompanyId(gprsConfigInfo.getCompanyId());
			queryGprsConfigInfo.setVolLevel(gprsConfigInfo.getVolLevel());
			List<GprsConfigInfo> updateList = gprsConfigInfoMapper.selectListByVolLevel(queryGprsConfigInfo);
//			List<GprsConfigInfo> updateList = selectListSelective(queryGprsConfigInfo);
		...
		if (flag) {
			// TODO ------- 远程主机特征测试动作参数 ----------------
			// updateByCompanyId 更据companyId(必填)和 deviceType(非必填)修改
...
//							continue;
//						}
						Integer type = nameAndcode.get(deviceTypeStr);
						subDeviceCount = nameAndSubCount.get(deviceTypeStr);
						volLevel = nameAndVol.get(deviceTypeStr);
						if(type != null && subDeviceCount != null && volLevel != null) {
							createGprsConfigInfo.setDeviceType(type);
							createGprsConfigInfo.setSubDeviceCount(subDeviceCount);
						}else {
	...
	public void isSubDevice(String gprsId, Integer deviceType, String gprsSpec,Integer subDeviceCount) {
		...
		if(count != 0 && count < subDeviceCount) {//原来是设备是4个从机，现在设备是24个从机，就增加后面的
			createSubDevice(gprsId, deviceType, gprsSpec,count,subDeviceCount);
		}
...
//						}
	...
	public void updateParamById(GprsConfigInfo gprsConfigInfo) {
		...
		BeanUtils.copyProperties(gprsConfigInfo, gprsConfigSend);
		gprsConfigSend.setId(null);
		//2018-3-26 添加逻辑 -----start------
		//如果页面标称容量和当前容量不传值：读取这个设备最近一次模型计算的容量（pack_data_expand_latest中的pack_cap_pred)，作为当前容量和标称容量写入
		//如果没有最近一次模型计算的容量：读取电池组的标称容量，电池组类型中的容量，作为标称和当前容量写入。
//		if(gprsConfigInfo.getGprsId() != null) {
//			PackDataExpandLatest packDataExpandLatest = new PackDataExpandLatest();
//			packDataExpandLatest.setGprsId(gprsConfigInfo.getGprsId());
//			List<PackDataExpandLatest> packDataExpandLatestList= packDataExpandLatestMapper.selectListSelective(packDataExpandLatest);
//			if(CollectionUtils.isNotEmpty(packDataExpandLatestList)) {
//				gprsConfigSend.setCurrentCapacity(new BigDecimal(packDataExpandLatestList.get(0).getPackCapPred()));
//				gprsConfigSend.setNominalCapacity(new BigDecimal(packDataExpandLatestList.get(0).getPackCapPred()));
//			}else {
//				StationInfo stationInfo = new StationInfo();
//				stationInfo.setGprsId(gprsConfigInfo.getGprsId());
//				List<StationInfo> stationInfoList = stationInfoSer.selectListSelective(stationInfo);
//				if(CollectionUtils.isNotEmpty(stationInfoList)) {
//					double packType = getStandardCapacityFromPackType(stationInfoList.get(0).getPackType());
//					gprsConfigSend.setCurrentCapacity(new BigDecimal(packType));
//					gprsConfigSend.setNominalCapacity(new BigDecimal(packType));
//				}else {//若设备没有绑定电池组默认值为500
//					gprsConfigSend.setCurrentCapacity(new BigDecimal(500));
//					gprsConfigSend.setNominalCapacity(new BigDecimal(500));
//				}
//			}
//		}
		//------2018-3-26 end-------
	...
	public double getStandardCapacityFromPackType(String packType) {
		// 48V500AH
		packType = packType.trim().toUpperCase();
		int StartIndexOf = packType.indexOf("V")+1;
		return ConvertUtils.toDouble(packType.substring(StartIndexOf, packType.indexOf("AH")),
				500d);
	}
