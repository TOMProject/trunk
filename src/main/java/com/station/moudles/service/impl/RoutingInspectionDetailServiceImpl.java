			...
			if (0 == row.getCell(5).getCellType()) {
				if (DateUtil.isCellDateFormatted(row.getCell(5))) {
					...
					if(date.getTime()> new Date().getTime()) {
						throw new RuntimeException("操作日期填写超过了当前时间！");
					}
...
//		}
		for (Integer index : cellIndexs) {
			...
			// 在cell_info表中设置对应的单单体为故障fault_mark = 1
			CellInfo cellInfo = new CellInfo();
			cellInfo.setStationId(station.get(0).getId());
			cellInfo.setCellIndex(index);
			cellInfo.setFaultMark(1);//故障
			cellInfo.setMarkTime(row.getCell(5).getDateCellValue());//操作时间为标记异常时间
			cellInfoMapper.updateCellInfoByStationIdAndCellIndex(cellInfo);
		}
	}
	...
	public List<Integer> recordCell(Row row, Integer cellNumer,Integer subCount) {
		...
		if (StringUtils.getString(row.getCell(cellNumer)) != null) {
			try {
				...
				throw new RuntimeException("故障单体编号填写格式不正确，如果设备类型是12V监测设备，那么单体的序号只有1-4，其它的设备类型是：1,2,3...24 整数  !注意是英文逗号或者中文逗号分隔开");
			}
		}
		return cellIndexs;
		...
		for (int i = 0; i < 24; i++) {
			if (!StringUtils.isNull(row.getCell(7 + 3 * i)) || !StringUtils.isNull(row.getCell(8 + 3 * i))
				...
				if (StringUtils.isNull(row.getCell(8 + 3 * i))) {
					throw new RuntimeException("编号" + (i + 1) + "确认有跟换的情况下，新换电池类型为必填项，且为‘新电池’或‘二次利旧’或‘活化电池’");
				} else {
					newType = RowParseHelper.getCell(row, 8 + 3 * i);
					if (!"二次利旧".equals(newType) && !"新电池".equals(newType) && !"活化电池".equals(newType)) {
						throw new RuntimeException("编号" + (i + 1) + "确认有跟换的情况下，新换电池的类型填写错误！正确类型为‘新电池’或‘二次利旧’或‘活化电池’");
					}
				}
	...
	public void confirmRoutingInspection(RoutingInspectionDetail routingInspectionDetail) {
		if (routingInspectionDetail.getRoutingInspectionsId() != null) {
			...
			if(routingCellinfo.size() != 0) {
				...
				inspectSignCellIndexSer.insert(inspectSignCell);
				CellInfo cellInfo = new CellInfo();
				cellInfo.setStationId(routingInspec.getStationId());
				cellInfo.setCellIndex(index);
				cellInfo.setFaultMark(1);//故障
				cellInfoMapper.updateCellInfoByStationIdAndCellIndex(cellInfo);
				}
