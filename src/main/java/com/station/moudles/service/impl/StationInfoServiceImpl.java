...
public class StationInfoServiceImpl extends BaseServiceImpl<StationInfo, Integer> implements StationInfoService {
	...
	public StationDetail getStationDetailByStationId(Integer stationId) {
		...
		// 增加的逻辑，浮冲状态没找到，得到最近的任意一条数据
		if (pdList.isEmpty()) {
			paramMap.put("state", null);
			pdList = packDataInfoMapper.selectByOrderSelective(paramMap);
		}
	...
	@Override
	public boolean parseStationExcelFile(File file, AjaxResponse ajaxResponse, Integer companyId,HttpServletRequest request)
		...
		try {
			...
			for (Row row : sheet) {
				...
				if (rowNum >= 2) {
					if (!RowParseHelper.hasData(row, 17)) {
						// 解析当前行有没有数据
						break;
					}
					...
					if (StringUtils.getString(row.getCell(4)) != null) {
						String addressName = RowParseHelper.getCell(row, 4).trim();
						Pattern regex = Pattern.compile("^[\\u4e00-\\u9fa5_a-zA-Z0-9]{1,30}$");
						Matcher matcher = regex.matcher(addressName);
						boolean flag = matcher.matches();
						if (flag) {
							stationInfo.setName(addressName);
						} else {
							errorRow.put(rowNum + 1, "物理地址信息必须是由中外、数字、下划线或者字符组成并且不超过30个字符！");
							continue;
						}
					} else {
					...
					if (StringUtils.getString(row.getCell(14)) != null) {
						...
						if (11 >= temp.length()) {
							if (!StringUtils.isNull(temp)) {
								CellVolLevel cellVolLevel = new CellVolLevel();
								cellVolLevel.setVolLevelName(temp);
								List<CellVolLevel> cellVol = cellVolLevelSer.selectListSelective(cellVolLevel);
								if(cellVol.size() == 0) {
									errorRow.put(rowNum + 1, "单体电压平台不存在！");
									continue;
								}
								if (temp.contains("V")) {
					...
					if(StringUtils.getString(row.getCell(15)) != null) {
						String loacCurrent = RowParseHelper.getCell(row, 15);
						Pattern regex = Pattern.compile("^(([1-9]{1,2})|([1-2]\\d{1,2})|300)(\\.\\d{1,3})?$");
						Matcher matcher = regex.matcher(loacCurrent);
						boolean matches = matcher.matches();
						if(!matches) {
							errorRow.put(rowNum + 1, "负载电流填写必须是：整数部分不超过0-300和小数部分不超过3位小数！ 例：200、21.002");
							continue;
						}else {
							stationInfo.setLoadCurrent(new BigDecimal(loacCurrent));
						}
					}
					
					//电池品牌
					if (StringUtils.getString(row.getCell(16)) != null) {
						String plant = RowParseHelper.getCell(row, 16);
						if (30 >= plant.length()) {
					...
					try {
						createStationCells(stationInfo,request);
					} catch (Exception e) {
	...
	@Override
	public void createStationCells(StationInfo stationInfo,HttpServletRequest request) {
	...
	public void updateGprsAndCellAndStation(StationInfo stationInfo) {
		//添加判断电池组的状态，电池组的状态必须是已安装或者未安装才能解绑
		StationInfo info = stationInfoSer.selectByPrimaryKey(stationInfo.getId());
		if(info != null) {
			if(info.getInspectStatus() != 99 && info.getInspectStatus() != 30) {
				throw new IllegalArgumentException("操作失败，其它用户在使用该电池组！");
			}
		}
	...
	public void detailInfo(StationDetail stationDetail, StationInfo stationInfo) {
		...
		if (!gprsId.equals("-1")) {
			...
			//获取平均内阻值
			BigDecimal[] avgResist = getAvgResist(gprsId);
			...
			for (CellInfo cell : cellList) {
				...
				if (pde != null) {
					cellDetail.setCellCapIndex(
							(Integer) BeanValueUtils.getValue("cellCapSort" + cellDetail.getCellIndex(), pde));
					//cellDetail.setCellResist((BigDecimal) BeanValueUtils.getValue("cellResist" + cellDetail.getCellIndex(), pde));
					cellDetail.setCellResist(avgResist[cellDetail.getCellIndex()-1]);
					cellDetail.setCellStatus(
							(Integer) BeanValueUtils.getValue("cellEvalu" + cellDetail.getCellIndex(), pde));
					cellDetail.setCellCap(
							(BigDecimal) BeanValueUtils.getValue("cellCap" + cellDetail.getCellIndex(), pde));
...
//		}
		...
		if(dviceType == null) {
			throw new RuntimeException("该设备的设备类型对应的从机个数没有设置！");
		}
	...
	@Override
	public List<StationInfo> selectStationInfoListFirst(StationInfo stationIfno) {
		
		return stationInfoMapper.selectStationInfoListFirst(stationIfno);
	}
	
	@Override
	public int selectListCountSelective(StationInfo record) {
		return stationInfoMapper.selectListCountSelective(record);
	}
	 ...
	 * @return
	 */
	public BigDecimal[]  getAvgResist(String gprsId){
		//判断redis 中是否存在gprsid对应的数据
		//获得缓存(srv_stationDetaile_resist_gprsid)		
		BigDecimal[] cacheArray = (BigDecimal[]) redisCacheAgent.getCache(Constant.AVGRESIST_PATH+gprsId);
		if(cacheArray != null) {
			return cacheArray;
		}
		StationInfo stationInfo = new StationInfo();
		...
		Integer cellCount = 24;
		if(stationInfos.size() != 0) {
			cellCount = stationInfos.get(0).getCellCount();
		}
		BigDecimal[] avgResist = new BigDecimal[cellCount];
		// 前台内阻显示取packDataExpand表里面的最新的10条数据， 去掉重复，算平均值
		PackDataExpand pack = new PackDataExpand();
		pack.setGprsId(gprsId);
		List<PackDataExpand> pde10 = packDataExpandSer.selectList10(pack);
		if(pde10.size() != 0) {
			for (int i = 0; i < pde10.size() - 1; i++) {
				for (int j = i + 1; j < pde10.size(); j++) {
					BigDecimal[] resistArrysI = new BigDecimal[cellCount];
					for (int k = 0; k < cellCount; k++) {
						resistArrysI[k] = (BigDecimal) BeanValueUtils.getValue("cellResist" + (k + 1), pde10.get(i));
					}
	
					BigDecimal[] resistArrysJ = new BigDecimal[cellCount];
					for (int k = 0; k < cellCount; k++) {
						resistArrysJ[k] = (BigDecimal) BeanValueUtils.getValue("cellResist" + (k + 1), pde10.get(j));
					}
					boolean isCount = true;
					for (int m = 0; m < cellCount; m++) {
						if (resistArrysI[m].compareTo(resistArrysJ[m]) != 0) {
							isCount = false;
							break;
						}
					}
			...
			for(int i = 0;i < cellCount; i++) {
				avgResist[i] = new BigDecimal(-1);
				logger.info("pack_data_expend表中没查询出来数--》{}",gprsId);
			}
			return avgResist;
		}
		Map<Integer, BigDecimal[]> resistData = new HashMap<Integer, BigDecimal[]>();

	
		if (pde10.size() != 0) {
			for (int i = 0; i < pde10.size(); i++) {
				BigDecimal[] resistArrys;
				resistArrys = new BigDecimal[cellCount];
				for (int j = 0; j < cellCount; j++) {
					resistArrys[j] = (BigDecimal) BeanValueUtils.getValue("cellResist" + (j + 1), pde10.get(i));
				}
				resistData.put(i, resistArrys);
			}

			// 算平均值
			for (int k = 0; k < cellCount; k++) {
				BigDecimal value = new BigDecimal(0.0);
				for (int i = 0; i < resistData.size(); i++) {
					value = value.add(resistData.get(i)[k]);
				}
				BigDecimal avg = value.divide(new BigDecimal(resistData.size()),3,BigDecimal.ROUND_HALF_UP);
				avgResist[k] = avg;
			}
		}
		//设置值为-1 前端不显示
		...
		//添加缓存	
		redisCacheAgent.refreshCache(Constant.AVGRESIST_PATH+gprsId, avgResist);
		return avgResist;

	}

	@Override
	public List<StationInfo> selectBatteryGroupWithYesterdayForIds(List<Integer> ids) {
		
		return stationInfoMapper.selectBatteryGroupWithYesterdayForIds(ids);
	}

	@Override
	public List<StationInfo> selectBatteryGroupEveryDay(Map map) {
		
		return stationInfoMapper.selectBatteryGroupEveryDay(map);
	}
